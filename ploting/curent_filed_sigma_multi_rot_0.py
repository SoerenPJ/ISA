...
STRUCTURES = [
    {
        "sweep_dir": "/home/soeren/University/masters/2.semester/ISA/scr/data_LLM/sweep_data_mu_armchair_bowtie_2x2_rot90",
        "mu":        3.36,
        "level":     "L2",
        "label":     "armchair bowtie 2x2",
        "E_angle_deg": 0,
        "a_cc_au":   2.6825,
    },
    {
        "sweep_dir": "/home/soeren/University/masters/2.semester/ISA/scr/data_LLM/sweep_data_mu_zigzag_bowtie_2x2_rot0",
        "mu":        3.36,
        "level":     "L2",
        "label":     "zigzag bowtie 2x2",
        "E_angle_deg": 0,
        "a_cc_au":   2.6825,
    },
    {
        "sweep_dir": "/home/soeren/University/masters/2.semester/ISA/scr/data_LLM/sweep_data_mu_armchair_triangle_2x2_rot90",
        "mu":        3.36,
        "level":     "L2",
        "label":     "Armchair triangle 2x2",
        "E_angle_deg": 0,
        "a_cc_au":   2.6825,
    },
    {
        "sweep_dir": "/home/soeren/University/masters/2.semester/ISA/scr/data_LLM/sweep_data_mu_zigzag_triangle_2x2_rot0",
        "mu":        3.36,
        "level":     "L2",
        "label":     "zigzag triangle 2x2",
        "E_angle_deg": 0,
        "a_cc_au":   2.6825,
    },
]
...
FONT_BASE     = 18   # was 16
FONT_TITLE    = 20
FONT_SCALEBAR = 18   # was 16
FONT_EFIELD   = 16   # was 13
FONT_LEGEND   = 13   # was 9
...
for row, struct in enumerate(STRUCTURES):
    ...
    if bonds_raw is not None:
        ...
        if row == 0: 
            cb.ax.set_title(r"$|J_{ll'}|$"  + r"(a.u.)", fontsize=FONT_LEGEND, pad=3)
    ...
    cb.ax.tick_params(labelsize=FONT_BASE - 2)
    if row == 0:
        cb.ax.set_title(r"$B_{\mathrm{ind},z}$" +  r"(a.u.)", fontsize=FONT_LEGEND, pad=3)
    ...
    SIGMA_COLORS = ['#08306b', '#2171b5', '#6baed6']
    B_COLORS     = ['#67000d', '#cb181d', '#fc9272']
    ...
    for idx, (mu_val, s, freq_t, b_max_t) in enumerate(trio):
        ...
        # ── σ_ext — solid, left axis
        ln, = ax.plot(o_eV, sv, '-', color=sc_col, lw=2.0,
                      label=rf"$\mu={mu_val:.1f}$ eV")
        ax.fill_between(o_eV, sv, alpha=0.08, color=sc_col)
        sigma_line_handles.append(ln)

        

        # ── B_ind,z — dashed, right axis (linear)
      
        mask   = freq_t <= omega_max
        b_vals = b_max_t[mask]
        b_line, = ax2.plot(freq_t[mask], b_vals * 1e8, '--',
                               color=bc_col, lw=2.0, label='_nolegend_')

            

    # ── Left axis: σ physical, linear
    ax.set_ylabel(r"$\sigma_\mathrm{ext}/A$",
                  color=SIGMA_COLORS[0], fontsize=FONT_BASE)
    ax.tick_params(axis='y', labelcolor=SIGMA_COLORS[0],
                   labelsize=FONT_BASE - 2)
    ax.tick_params(axis='x', labelsize=FONT_BASE - 2)
    ax.set_xlabel(r"$\hbar\omega$ (eV)", fontsize=FONT_BASE)
    ax.set_ylim(bottom=0)

    # ── Right axis: B linear
    ax2.set_ylabel(r"$|B_{\mathrm{ind},z}|$ ($\times10^{-8}$ a.u.)",
                   color=B_COLORS[0], fontsize=FONT_BASE)
    ax2.tick_params(axis='y', labelcolor=B_COLORS[0],
                    labelsize=FONT_BASE - 2)

    # ── Single legend: μ values only
    ax.legend(handles=sigma_line_handles,
...
#  SAVE
# ============================================================
out = "multi_structure_panels_rot0_vol1.png"
