...
STRUCTURES = [
    ...
    "/home/soeren/University/masters/2.semester/ISA/data_LLM/sweep_data_mu_zigzag_bowtie_15x15_rot90",
    "/home/soeren/University/masters/2.semester/ISA/data_LLM/sweep_data_mu_armchair_triangle_14x14_rot0",
    "/home/soeren/University/masters/2.semester/ISA/data_LLM/sweep_data_mu_zigzag_triangle_22x22_rot90",  # ← replace
]
...
MU_CASES_PER_STRUCTURE = {
    ...
    "sweep_data_mu_zigzag_bowtie_15x15_rot90":    [0.5, 2.0, 3.5],
    "sweep_data_mu_armchair_triangle_14x14_rot0": [0.5, 2.0, 3.5],
    "sweep_data_mu_zigzag_triangle_22x22_rot90":   [0.5, 2.0, 3.5],  
}
...
def _draw_scalebar(ax, x, y, xpad, ypad, arrow_style):
    ...
    # ── Auto-detect longest side for "horizontal" style ──────────────────
    if arrow_style == "horizontal":
        arrow_style = "horizontal_x" if true_w >= true_h else "horizontal_y"

    if arrow_style in ("horizontal_x", "bowtie_h"):
        # Arrow along x-axis (full width), placed below the structure
    ...
    elif arrow_style == "horizontal_y":
        # Arrow along y-axis (full height), placed to the left of the structure
        bar_len  = round(true_h, 1)
        bar_x    = x.min() - xpad * 0.55
        bar_ymid = (y.min() + y.max()) / 2
        bar_ylo  = bar_ymid - true_h / 2
        bar_yhi  = bar_ymid + true_h / 2
...
def draw_eigenvalue_spectrum(ax, energies_eV, gap_bot, gap_top, gap,
    ...
    for mu_val, color, label in sorted(
        ...
        ax.axhline(mu_val, color=color, lw=1.5, ls='--', alpha=0.8, zorder=4,  rasterized=True)
...
out = "linear_response_rot_90_vol2.png"
